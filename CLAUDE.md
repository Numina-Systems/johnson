...
- **Go TUI** (`tui/`): Bubble Tea terminal UI that communicates with the TS backend over JSON-RPC 2.0 via stdin/stdout. The Go binary spawns the TS backend as a child process (`bun run src/index.ts --interface jsonrpc`), waits for a `ready` notification, then drives all interaction through RPC calls. See `tui/CLAUDE.md` for architecture details. Same 6-screen layout as the legacy TUI: Sessions, Chat, Tools, Secrets, Schedules, SystemPrompt. Navigation via slash commands from the chat input: `/tools`, `/secrets`, `/schedules`, `/prompt`, `/back`, `/quit`. `escape` goes back, `ctrl+c` quits. Discord coexistence: `--with-discord` flag spawns the backend in `both` mode.
